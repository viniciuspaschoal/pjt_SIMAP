...
export default function FiltroProjeto({ onApplyFilters }) {
    ...
    const aplicarFiltros = async () => {
        ...
        console.log("Payload enviado:", filtros);

        try {
            const response = await aplicarFiltrosProjeto(filtros);
            console.log("Resposta do servidor:", response.data);

            // ATUALIZAÇÃO: Passamos os DADOS (response.data) e os FILTROS (filtros)
    ...
    // Componente de Lista (Escolas/Professores) com controle de fonte
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full min-h-[250px]">
                <div className="p-3 border-b bg-slate-50/60 shrink-0">
                    <div className="flex items-center justify-between">
                        <h3 className="font-semibold text-base uppercase tracking-wide text-slate-700">{title}</h3>
                        {selected.length > 0 && (
                ...
                <div className="flex-1 p-3 space-y-1 overflow-y-auto custom-scroll">
                    {searchTerm === "" && (
                        <label className="flex items-center gap-3 p-2 rounded-lg cursor-pointer hover:bg-slate-50">
                            ...
                            <span className="font-medium text-sm text-slate-600">{isAllSelected ? "Desmarcar Todas" : "Selecionar Todas"}</span>
                        </label>
                    )}
                    {filteredItems.map(item => {
                        ...
                        return (
                            <label key={item} className="flex items-center gap-3 p-2 rounded-lg cursor-pointer hover:bg-slate-50">
                                ...
                                {/* AQUI É ONDE O TAMANHO DA FONTE É APLICADO */}
                                <span className={`${itemTextClass} ${isSelected ? "font-semibold text-emerald-700" : "text-slate-700"}`}>{item}</span>
                            </label>
                        );
                    })}
    ...
    // Componente de Tags (Anos, Meses, etc) com controle de fonte
    const TagSection = ({ title, items, selected, setSelected, gridCols, className = "", itemTextClass = "text-xs" }) => (
        <div className={`bg-white rounded-xl shadow-sm border border-slate-200 p-4 shrink-0 ${className}`}>
            <div className="flex justify-between items-center mb-3">
                <h3 className="font-semibold text-base uppercase tracking-wide text-slate-700">{title}</h3>
                {selected.length > 0 && (
                    <button onClick={() => setSelected([])} className="text-sm text-red-500 hover:text-red-600">Limpar</button>
                )}
            </div>
                    ...
                    return (
                        <button
                            ...
                            className={`
                                ...
                                ${itemTextClass} 
                                ${isSelected ? "bg-emerald-500 text-white border-emerald-500 shadow" : "bg-white text-slate-600 border-slate-300 hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-400"}
                            `}
                        >
                            {item}
                        </button>
            ...
            <div className="shrink-0 px-8 pt-6 pb-4">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    ...
                    <button
                        ...
                        className="px-6 py-3 bg-[#0a7c41] hover:bg-[#004d25] text-white font-semibold rounded-xl flex items-center gap-2 transition-all transform hover:scale-105"
                    >
            ...
            <div className="flex-1 overflow-hidden mx-8 bg-gray-100 max-h-[75vh] p-2 pb-3 rounded-md border border-gray-200">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-full">
                    ...
                    <div className="lg:col-span-4 h-full overflow-y-auto custom-scroll pr-2 flex flex-col gap-4">
                        ...
                        {/* --- EXIBIÇÃO DE TURNO (FONTE PADRÃO PEQUENA) --- */}
                        <TagSection
                            title="Turno"
                            items={turnos}
                            selected={selTurnos}
                            ...
                            gridCols="grid-cols-1"
                            itemTextClass="text-sm" /* <-- MUDE AQUI O TAMANHO */
                        />
                    </div>
                    ...
                    <div className="lg:col-span-8 h-full overflow-y-auto custom-scroll flex flex-col gap-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1 min-h-[300px]">
                            ...
                            {/* --- LISTA DE ESCOLAS (FONTE PEQUENA P/ CABER MAIS) --- */}
                            <FilterListSection
                                title="Escolas"
                                items={escolas}
                                selected={selEscolas}
